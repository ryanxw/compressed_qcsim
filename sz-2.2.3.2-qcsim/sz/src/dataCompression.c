...
void computeRangeSize_double2(double* oriData, size_t size, double* valueRangeSize1, double* valueRangeSize2, double* medianValue1, double* medianValue2, double* min1, double* min2)
{
	size_t i = 0;
	*min1 = oriData[0];
	*min2 = oriData[1];
	...
	*valueRangeSize1 = max1 - *min1;
	*valueRangeSize2 = max2 - *min2;
	*medianValue1 = *min1 + *valueRangeSize1/2;
	*medianValue2 = *min2 + *valueRangeSize2/2;
}
...
inline void computeNonZeros(double* oriData, size_t size, size_t t_nbNonZeros, size_t* r_nbNonZeros, double** nonZeros, unsigned int** nonZeros_loc)
{
	size_t i = 0;
    *nonZeros = (double*)malloc(sizeof(double)*(t_nbNonZeros+1));
    memset(*nonZeros, 0, sizeof(double)*(t_nbNonZeros+1));
    
    *nonZeros_loc = (unsigned int*)malloc(sizeof(unsigned int)*(t_nbNonZeros+1));
    memset(*nonZeros, 0, sizeof(unsigned int)*(t_nbNonZeros+1));

	double* p = *nonZeros;
	unsigned int* q = *nonZeros_loc;
	*r_nbNonZeros = 0; //real number of non-zero values        	
	
	for(i=0;i<size;i++)
	{
		double data = oriData[i];
		
        if(*r_nbNonZeros < t_nbNonZeros+1 && data!=0)
        {
			(*r_nbNonZeros) ++;
			*(p++) = data;
			*(q++) = i;
...
double computeRangeSize_double_MSST19_qcsim(double* oriData, size_t size, double* valueRangeSize, double* medianValue, unsigned char * signs, bool* positive, double* nearZero)
{
    size_t i = 0;
    double min = oriData[0];
    double max = min;
    *nearZero = min;
    ...
    for(i=1;i<size;i++)
    {
        ...
        if(data != 0 && fabs(oriData[i]) < fabs(*nearZero)){
            *nearZero = oriData[i];
        }
        if(min>data)
            min = data;
...
inline void compressSingleDoubleValue_MSST19_2(DoubleValueCompressElement *vce, double tgtValue, double precision, int reqLength, int reqBytesLength, int resiBitsLength)
{
    ldouble lfBuf;
