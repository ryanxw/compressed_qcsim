...
int new_TightDataPointStorageD_fromFlatBytes(TightDataPointStorageD **this, unsigned char* flatBytes, size_t flatBytesLength)
{
	...
	(*this)->nbNonZeroData = (sameRByte & 0x80) >> 7;
	...
	if((*this)->nbNonZeroData)
	{
		//size_t exactMidBytesLength = flatBytesLength - 3 - 1 - MetaDataByteLength - exe_params->SZ_SIZE_TYPE*2;
		
		for (i = 0; i < exe_params->SZ_SIZE_TYPE; i++)
			dsLengthBytes[i] = flatBytes[index++];
		(*this)->nbNonZeroData = bytesToSize(dsLengthBytes);
...
void convertTDPStoFlatBytes_double(TightDataPointStorageD *tdps, unsigned char** bytes, size_t *size) 
{
	...
	unsigned char sameByte = tdps->allSameData==1?(unsigned char)1:(unsigned char)0; //00000001
	sameByte = sameByte | (confparams_cpr->szMode << 1);
	if(tdps->isLossless)
		sameByte = (unsigned char) (sameByte | 0x10); // 00010000, the 4th bit
	if(confparams_cpr->errorBoundMode>=PW_REL)
	...
	if(confparams_cpr->errorBoundMode == PW_REL && confparams_cpr->accelerate_pw_rel_compression)
		sameByte = (unsigned char) (sameByte | 0x04); // 00000100
	if(tdps->nbNonZeroData>0)
		sameByte = (unsigned char) (sameByte | 0x80); //100000000, the 7th bit
	...
	else if (tdps->nbNonZeroData>0)
	{
		size_t totalByteLength = 3 + 1 + MetaDataByteLength + exe_params->SZ_SIZE_TYPE*2 + tdps->exactMidBytes_size;
		*bytes = (unsigned char *)malloc(sizeof(unsigned char)*totalByteLength);
		...
		//store nbNonZeros
		if(exe_params->SZ_SIZE_TYPE==4)
			intToBytes_bigEndian(dsLengthBytes, tdps->nbNonZeroData);//4
		else
			longToBytes_bigEndian(dsLengthBytes, tdps->nbNonZeroData);//8		
		
		for (i = 0; i < exe_params->SZ_SIZE_TYPE; i++)
			(*bytes)[k++] = dsLengthBytes[i];
		
		//store non-zeros array
		for (i = 0; i < tdps->exactMidBytes_size; i++)
			(*bytes)[k++] = tdps->exactMidBytes[i];
