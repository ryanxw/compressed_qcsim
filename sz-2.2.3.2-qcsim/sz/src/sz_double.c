...
size_t dataLength, double realPrecision, double valueRangeSize, double medianValue_f)
{
	...
	//add the first data	
	type[0] = 0;
	compressSingleDoubleValue_MSST19_2(vce, spaceFillingValue[0], realPrecision, reqLength, reqBytesLength, resiBitsLength);
	updateLossyCompElement_Double(vce->curBytes, preDataBytes, reqBytesLength, resiBitsLength, lce);
	memcpy(preDataBytes,vce->curBytes,8);
	addExactData(exactMidByteArray, exactLeadNumArray, resiBitArray, lce);
	listAdd_double(last3CmprsData, vce->data);
	...
	//add the second data
	type[1] = 0;
	compressSingleDoubleValue_MSST19_2(vce, spaceFillingValue[1], realPrecision, reqLength, reqBytesLength, resiBitsLength);
	updateLossyCompElement_Double(vce->curBytes, preDataBytes, reqBytesLength, resiBitsLength, lce);
	memcpy(preDataBytes,vce->curBytes,8);
	addExactData(exactMidByteArray, exactLeadNumArray, resiBitArray, lce);
	listAdd_double(last3CmprsData, vce->data);
	...
	for(i=2;i<dataLength;i++)
	{
		...
		//unpredictable data processing
		type[i] = 0;
		compressSingleDoubleValue_MSST19_2(vce, curData, realPrecision, reqLength, reqBytesLength, resiBitsLength);
		updateLossyCompElement_Double(vce->curBytes, preDataBytes, reqBytesLength, resiBitsLength, lce);
		memcpy(preDataBytes,vce->curBytes,8);
		addExactData(exactMidByteArray, exactLeadNumArray, resiBitArray, lce);
		listAdd_double(last3CmprsData, vce->data);
...
void SZ_compress_args_double_KNonZeros(unsigned char** newByteData, size_t nbNonZeros, double* nonZeros, unsigned int* loc, size_t dataLength, size_t *outSize)
{
	int int_size = sizeof(unsigned int); //should be 4
	...
	tdps->nbNonZeroData = nbNonZeros;
	tdps->dataSeriesLength = dataLength;
	tdps->exactMidBytes = (unsigned char*)malloc(double_size*nbNonZeros+int_size*nbNonZeros);
	
	unsigned char* p = tdps->exactMidBytes;
	unsigned char* q = tdps->exactMidBytes+sizeof(double)*nbNonZeros;
	int i = 0;
	for(i=0;i<nbNonZeros;i++,p+=double_size,q+=int_size)
	{
		doubleToBytes(p, nonZeros[i]);
		intToBytes_bigEndian(q, loc[i]);
	}
	
	tdps->exactMidBytes_size = double_size*nbNonZeros+int_size*nbNonZeros;
	convertTDPStoFlatBytes_double(tdps, newByteData, outSize);
	free_TightDataPointStorageD(tdps);
}
...
int truncation_compression_args_double_qcsim(unsigned char** newByteData, double *oriData, size_t nbEle, size_t *outSize, double pwRelBoundRatio)
{
	short reqLength = computeReqLength_double_MSST19(pwRelBoundRatio);
	size_t i = 0;
	
	unsigned char *exactLeadNumArray = (unsigned char*)malloc(nbEle);
	
	unsigned char *exactMidByteArray = (unsigned char*)malloc(8*nbEle);
	unsigned char *mid = exactMidByteArray;
	
	unsigned char *resiBitArray = (unsigned char*)malloc(nbEle);
	...
	*newByteData = (unsigned char*)malloc(sizeof(double)*nbEle);
	unsigned char* v = *newByteData;
	//reshuffle
	double *tmp = (double *)malloc(sizeof(double)*nbEle);
	double *p = tmp, *q = tmp+nbEle/2;
	for(i=0;i<nbEle;i+=2)
	{
		*(p++) = oriData[i];
		*(q++) = oriData[i+1];
	...
	size_t midBytesCount = 0;
	double  curData = 0;
	for(i=0;i<nbEle;i++)
	{
		curData = tmp[i];
		compressSingleDoubleValue_MSST19_2(vce, curData, 0, reqLength, reqBytesLength, resiBitsLength);
		updateLossyCompElement_Double(vce->curBytes, preDataBytes, reqBytesLength, resiBitsLength, lce);
		memcpy(preDataBytes,vce->curBytes,8);
		//addExactData(exactMidByteArray, exactLeadNumArray, resiBitArray, lce);
		
		exactLeadNumArray[i] = lce->leadingZeroBytes;
		...
		if(intMidBytes!=NULL)
		{
			memcpy(mid, intMidBytes, lce->integerMidBytes_Length);
			mid += lce->integerMidBytes_Length;
			midBytesCount += lce->integerMidBytes_Length;
		}

		if(resMidBitsLength!=0)
			resiBitArray[i] =  lce->residualMidBits;
	}

	//start pushing bytes
	
	unsigned char *exactLeadNumBytes = NULL;
	//put bytes to *newByteData	
	size_t leadNumBytes_size = convertIntArray2ByteArray_fast_2b(exactLeadNumArray, nbEle, &exactLeadNumBytes);
	
	sizeToBytes(v, leadNumBytes_size);
	v+=sizeof(size_t);
	sizeToBytes(v, midBytesCount);
	v+=sizeof(size_t);
	shortToBytes(v, reqLength);
	v+=sizeof(short);
		
	memcpy(v, exactLeadNumBytes, leadNumBytes_size);
	v+=leadNumBytes_size;
	
	memcpy(v, exactMidByteArray, midBytesCount);
	v+=midBytesCount;
	
	unsigned char *residualMidBits = NULL;
	size_t residualMidBits_size = convertIntArray2ByteArray_fast_dynamic(resiBitArray, resiBitsLength, nbEle, &residualMidBits);
	memcpy(v, residualMidBits, residualMidBits_size);
	v+=residualMidBits_size;

	*outSize = 2*sizeof(size_t)+sizeof(short)+leadNumBytes_size+midBytesCount+residualMidBits_size;
	
	free(exactMidByteArray);
	free(exactLeadNumArray);
	free(exactLeadNumBytes);
	free(resiBitArray);
...
int truncation_compression_args_double_qcsim2(unsigned char** newByteData, double *oriData, size_t nbEle, size_t *outSize, double pwRelBoundRatio)
{
	short reqLength = computeReqLength_double_MSST19(pwRelBoundRatio);
	size_t i = 0;
...
int SZ_compress_args_double_qcsim(unsigned char** newByteData, double *oriData, 
size_t nbEle, size_t *outSize, 
int errBoundMode, double absErr_Bound, double relBoundRatio, double pwRelBoundRatio, size_t nbNonZeros, int max_quant_intervals)
{
	confparams_cpr->errorBoundMode = errBoundMode;
	if(errBoundMode==PW_REL)
	{
		//TODO if higher performance is needed
		confparams_cpr->max_quant_intervals = max_quant_intervals; //to be tuned
	...
	size_t r_nbNonZeros = 0;
	double* nonZeros = NULL;
	unsigned int* nonZeros_loc = NULL;	
	
	if(nbNonZeros>0)
	{
		computeNonZeros(oriData, dataLength, nbNonZeros, &r_nbNonZeros, &nonZeros, &nonZeros_loc);		
		if(r_nbNonZeros > 0 && r_nbNonZeros <= nbNonZeros) //just store the non-zeros data points directly
		{
			SZ_compress_args_double_KNonZeros(newByteData, r_nbNonZeros, nonZeros, nonZeros_loc, dataLength, outSize);
			free(nonZeros);
			free(nonZeros_loc);
			return status;
		}
	...
	if(confparams_cpr->errorBoundMode == PW_REL && confparams_cpr->accelerate_pw_rel_compression == 1)
	{
		...
		min = computeRangeSize_double_MSST19_qcsim(oriData, dataLength, &valueRangeSize, &medianValue, signs, &positive, &nearZero);
	}
	else
	...
	else
	{
		size_t tmpOutSize = 0;
		unsigned char* tmpByteData;	
		if(confparams_cpr->errorBoundMode>=PW_REL) //point wise relative error bound
		{
			if(confparams_cpr->accelerate_pw_rel_compression) //>=1E-5
				SZ_compress_args_double_NoCkRngeNoGzip_1D_pwr_pre_log_MSST19(&tmpByteData, oriData, pwRelBoundRatio, dataLength, &tmpOutSize, valueRangeSize, medianValue, signs, &positive, min, max, nearZero);
			else
				SZ_compress_args_double_NoCkRngeNoGzip_1D_pwr_pre_log(&tmpByteData, oriData, pwRelBoundRatio, dataLength, &tmpOutSize, min, max);
		}
		else //absolute error bound or value range based relative error bound
		{
			SZ_compress_args_double_NoCkRngeNoGzip_1D(&tmpByteData, oriData, dataLength, realPrecision, &tmpOutSize, valueRangeSize, medianValue);
		...
		//Call Zstd to do the further compression.
