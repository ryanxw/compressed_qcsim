...
int main(int argc, char * argv[])
{
    ...
    if(argc < 9)
    {
        printf("Test case: testdouble_CompDecomp_pastri [config_file] [zc.config] [solName] [varName] [errBoundMode] [ErrBound] [srcFilePath] [dimension sizes...]\n");
        printf("Example: testdouble_CompDecomp_pastri pastri.config zc.config pastri(1E-6) testdouble ABS 1E-6 testdata/x86/testdouble_8_8_128.dat 8 8 128\n");
        exit(0);
    }
    ...
    pastri_par.originalEb = absErrBound;
    ...
    ZC_startDec();
    double *decData = NULL;
    //decData = SZ_decompress(SZ_DOUBLE, bytes, outSize, r5, r4, r3, r2, r1);
	SZ_pastriDecompressBatch(bytes, &pastri_par, (unsigned char**)&decData, &nbEle);
    nbEle=nbEle/8;
