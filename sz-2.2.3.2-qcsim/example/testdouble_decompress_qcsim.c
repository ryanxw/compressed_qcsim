...
int main(int argc, char * argv[])
{
    ...
    size_t nbCpxData, totalNbEle;
    char zipFilePath[640], outputFilePath[640];
    
    if(argc < 2)
    {
		printf("Test case: testdouble_decompress_qcsim [srcFilePath] [dimension sizes...]\n");
		printf("Example: testdouble_decompress_qcsim testdouble_8_8_128.dat.sz 8 8 128\n");
		exit(0);
	}	
    ...
    if(r2==0)
        ...
        nbCpxData = r1*r2*r3*r4;
    else
        nbCpxData = r1*r2*r3*r4*r5;
    ...
    cost_start();    
    size_t nbEle = 2*nbCpxData;
    double *data = (double*)malloc(sizeof(double)*nbEle);
    
    SZ_decompress_qcsim(SZ_DOUBLE, bytes, byteLength, data, nbCpxData, 2);
    cost_end();    
    printf("timecost=%f\n",totalCost);
